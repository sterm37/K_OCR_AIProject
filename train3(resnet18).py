...
class DNN: #slp 에서 중간에 여러개의 히든 레이어를 집어 넣어 DNN을 구현
    ...
    def __init__(self, train_ds, valid_ds, epoch, batchSize, errRate, cLayer, imgSize, unit, path) -> None:
        ...
        self.dp = DirPath('18', batchSize, unit, imgSize) #경로 지정
    ...
    def ConvLayerAdd(self):#합성 곱 계층을 cLayer 변수 크기의 개수만큼 생성하는 함수 활성 함수는 relu 사용
        if self.cLayer > 0:
            ...
            self.md = self.ConvIden(self.md, 0)
            
            self.md = self.ConvBlock(self.md, 1)
            self.md = self.ConvIden(self.md, 1)
            
            self.md = self.ConvBlock(self.md, 2, 2)
            self.md = self.ConvIden(self.md, 2)
            
            self.md = self.ConvBlock(self.md, 3, 2)
            self.md = self.ConvIden(self.md, 3)
            
            self.md = tf.keras.layers.GlobalAveragePooling2D()(self.md)#분류를 위해 평탄화
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='RESNET18 구현')
    parser.add_argument('-bs', '--batch_size', help = '배치 사이즈', type=int, default=256)
    parser.add_argument('-er', '--final_err', help = '최종 도달 목표 에러율', type=float, default=0.0)
    ...
    parser.add_argument('-p', '--path', help = '모델 저장 경로', type=str, default='resnet18')
