...
class DNN: #slp 에서 중간에 여러개의 히든 레이어를 집어 넣어 DNN을 구현
    ...
    def __init__(self, train_ds, valid_ds, epoch, batchSize, errRate, cLayer, imgSize, unit, path) -> None:
        ...
        self.dp = DirPath('16', batchSize, unit, imgSize) #경로 지정
    ...
    def ConvLayerAdd(self):#합성 곱 계층을 cLayer 변수 크기의 개수만큼 생성하는 함수 활성 함수는 relu 사용
        if self.cLayer > 0:
            ...
            self.x = tf.keras.layers.Conv2D(self.unit/8, (3, 3), padding = 'same', activation='relu')(self.x)
            self.x = tf.keras.layers.Conv2D(self.unit/8, (3, 3), padding = 'same', activation='relu')(self.x)
            self.x = tf.keras.layers.MaxPooling2D(2,2)(self.x)
            ...
            self.x = tf.keras.layers.Dense(4096, activation = 'relu')(self.x)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='VGG16 구현')
    ...
    parser.add_argument('-p', '--path', help = '모델 저장 경로', type=str, default='vgg16')
