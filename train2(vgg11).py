...
class DirPath:
    ...
    def OutLog(self, fName, dataList):
        ...
        if os.path.isfile(fN) and fName=='Train': #학습 로그가 있으면 이어서 쓰고 없으면 생성한다
            file = open(fN, 'a')
        else:
            file = open(fN, 'w')
    ...
    #keras에 존재하는 모델 저장 함수        
    def SaveModel(self, model):
        model.save(self.modelPath)
        
    def LoadModel(self):# 모델을 불러오는 함수
        model = tf.keras.models.load_model(self.modelPath)
        
        return model
    
    def ErrSave(self, err): # 제일 정확도가 높았던 순간의 에러율을 기록
    ...
    def ErrLoad(self): #전 학습에서의 에러율 불러오기 없으면 생성하게 된다.
        fN = "{0}/{1}_Log_BatchSize{2}_imgsize{3}.txt".format(self.modelPath, 'Err', self.batchSize, self.img_size)
...
class DNN: #slp 에서 중간에 여러개의 히든 레이어를 집어 넣어 DNN을 구현
    ...
    cLayer = 0 #합성곱 레이어 개수
    img_size = 32 #이미지 사이즈 기본값
    train_ds = None #학습 데이터셋
    valid_ds = None #검증 데이터셋
    unit = 32
    ...
    def __init__(self, train_ds, valid_ds, epoch, batchSize, errRate, cLayer, imgSize, unit, path) -> None:
        ...
        self.img_size = imgSize #이미지사이즈 업데이트
        self.dp = DirPath('11', batchSize, unit, imgSize) #경로 지정
        self.train_ds = train_ds #학습데이터셋 저장
        self.valid_ds = valid_ds #검증 데이터셋 저장
        self.unit = unit #계층 별 유닛 배율 저장
        
        self.dp.MakeDirectory(path)#경로 생성
        
        self.ConvLayerAdd()#합성곱 계층 생성 (개수가 1 이상일 때)
    ...
    def ConvLayerAdd(self):#합성 곱 계층을 cLayer 변수 크기의 개수만큼 생성하는 함수 활성 함수는 relu 사용
        if self.cLayer > 0:
            ...
            self.x = tf.keras.layers.Rescaling(1./255)(modelInput) #0~1 사이의 값만 갖도록 변환
            
            self.x = tf.keras.layers.Conv2D(self.unit/8, (3, 3), padding = 'same', activation='relu')(self.x) #합성곱 계층 생성으로 근접 유닛들만 연결하게 된다.
            self.x = tf.keras.layers.MaxPooling2D(2,2)(self.x) #이미지 크기를 반으로 줄이게 된다
            
            self.x = tf.keras.layers.Conv2D(self.unit/4, (3, 3), padding = 'same', activation='relu')(self.x)
            self.x = tf.keras.layers.MaxPooling2D(2,2)(self.x) #해당 부분에서 다시 이미지 크기를 반으로 줄이게 된다
            
            self.x = tf.keras.layers.Conv2D(self.unit/2, (3, 3), padding = 'same', activation='relu')(self.x)
            self.x = tf.keras.layers.Conv2D(self.unit/2, (3, 3), padding = 'same', activation='relu')(self.x)
            self.x = tf.keras.layers.MaxPooling2D(2,2)(self.x) #해당 부분에서 다시 이미지 크기를 반으로 줄이게 된다
            
            self.x = tf.keras.layers.Conv2D(self.unit, (3, 3), padding = 'same', use_bias=False)(self.x) #BN계층에서 베타 값이 bias값을 대체한다.
            self.x = tf.keras.layers.BatchNormalization()(self.x) #파라미터 값이 일정범위 안으로 조절하게 한다
            self.x = tf.keras.layers.Activation('relu')(self.x)
            self.x = tf.keras.layers.Conv2D(self.unit, (3, 3), padding = 'same', use_bias=False)(self.x)
            ...
            self.x = tf.keras.layers.Dense(2048, activation = 'relu')(self.x) #2048개의 유닛과 위 결과값을 전체 연결(Fully Connect)한다
            self.x = tf.keras.layers.Dropout(.5)(self.x) #해당 계층을 통해 유닛들을 50%의 비율로 비활성화 시킨다
            self.x = tf.keras.layers.Dense(2048, activation = 'relu')(self.x)
            self.x = tf.keras.layers.Dropout(.5)(self.x)
    ...
    def OutTestLog(self, x):
        ...
        testList.append('accuracy_score: {}'.format(accuracy_score(y2, y1))) #정확도 계산
        testList.append('recall_score: {}'.format(recall_score(y2, y1, average='micro'))) #재현율 계산 tp/tp+fn
        testList.append('precision_score: {}'.format(precision_score(y2, y1, average='micro'))) #정밀도 계산 tp / tp+fp
        testList.append('f1_score: {}'.format(f1_score(y2, y1, average='micro'))) # F1 Score 식 계산 2 * ((prec*recall)/(prec+recall))
    ...
    def getStrSummary(self): #해당 모델 구조와 파라미터 개수를 불러와 텍스트 파일에 저장하게 된다.
        sumList = []
    ...
    def saveModel(self): #모델 저장
        self.dp.SaveModel(self.model)#경로에 모델 저장
    
    def getModel(self): #모델 불러오기
        self.model = self.dp.LoadModel()
        
    def saveErr(self, err): #에러율 저장
        self.dp.ErrSave(err)
        
    def loadErr(self): #에러율 불러오기
        return self.dp.ErrLoad()
    
    def saveparam(self, path): #가중치 저장
        self.model.save_weights('./models/'+path+'/weights.h5', overwrite = True)

    def loadparam(self, path): #가중치 불러오기
...
def main(batch_Size, finalErr, epoch, cLayer, image_size, unit, path, act):
    ...
    if act == 'test': #train이면 학습, test이면 테스트검증, 잘못된 값은 파이썬 프로그램 종료
    ...
    #categorical_crossentropy를 통해 entropy를 이용한 두 데이터간의 차이를 계산하게 된다.
    dnn.CompModel(optimizer, 'categorical_crossentropy', ['accuracy'])
    err = 1.0
    
    if len(os.listdir('./models/{}'.format(path))) > 0: #해당 폴더에 파일이 존재하는가 존재하면 가중치 불러오기
        dnn.loadparam(path)
        err = dnn.loadErr()
    
    if act == 'test': #목적이 테스트 인가 학습인가
        ...
        dnn.OutTrainData() #학습 로그 출력
        dnn.PlotModel(path) #모델 구조 이미지 저장

if __name__ == "__main__":
    #배치스크립트를 실행하기 위한 arguments를 받아오는 부분 없으면 기본값으로 실행이 됩니다.
    parser = argparse.ArgumentParser(description='VGG11 구현')
    parser.add_argument('-bs', '--batch_size', help = '배치 사이즈', type=int, default=256)
    ...
    parser.add_argument('-cl', '--c_layer', help = '은닉 계층 개수(conv2d,conv2d,pooling)', type=int, default=1)
    parser.add_argument('-img', '--image_size', help = '이미지 사이즈', type=int, default=32)
    parser.add_argument('-u', '--unit', help = '한 계층당 유닛 크기 배율', type=int, default=512)
    parser.add_argument('-p', '--path', help = '모델 저장 경로', type=str, default='vgg11')
